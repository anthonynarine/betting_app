...
const useAxiosWithInterceptorJwt = () => {
  ...
  const jwtReqAxios = axios.create({ baseURL: API_BASE_URL });
  ...
  jwtReqAxios.interceptors.response.use(
    (response) => {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        if (refreshToken) {
          try {
            ...
            return jwtReqAxios(originalRequest);
...
export default useAxiosWithInterceptorJwt;
